...
with open("sources/fpl.team.players.tsv", mode="r") as file:
    r = csv.reader(file, delimiter="\t")
    for team, position, player, cost in r:
        cost = float(cost)
        if team in fplteam:
            if position in fplteam[team]:
                fplteam[team][position].append((player, cost))
            else:
                fplteam[team][position] = [(player, cost)]
        else:
            fplteam[team] = {position: [(player, cost)]}
...
for team, team_values in fplteam.items():
